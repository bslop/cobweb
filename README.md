...
| **jas** | JRISC assembler that refuses to assemble lies: hazard checking as errors, real diagnostics with fix-its, sections/relocations with SRAM overlay groups as first-class objects, rmac compat mode | planned |
| **jtest** | Shadow harness + golden vectors: new code and reference run side by side on device/sim, byte-parity gates, differential runs across sim profiles / BigPEmu / hardware | planned |
